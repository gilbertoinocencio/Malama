...
export const RefinePlan: React.FC<RefinePlanProps> = ({ onBack, onNavigate }) => {
  ...
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto shadow-2xl bg-Malama-bg dark:bg-background-dark font-display transition-colors duration-200 text-Malama-main dark:text-white animate-fade-in">
      ...
      <div className="flex items-center bg-transparent p-4 pb-2 justify-between sticky top-0 z-20 transition-colors duration-200 backdrop-blur-sm">
        <div onClick={onBack} className="text-Malama-petrol dark:text-white flex size-12 shrink-0 items-center justify-start cursor-pointer transition-opacity hover:opacity-70">
          <span className="material-symbols-outlined text-2xl">arrow_back</span>
        </div>
        <h2 className="text-Malama-petrol dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center pr-12">{rp.title}</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
        ...
        <div className="px-5 pt-6 pb-2 animate-fade-in-up">
          ...
          <p className="text-Malama-muted dark:text-gray-400 text-sm font-normal leading-relaxed pt-2">{rp.aiAnalysis}</p>
        </div>
            ...
            {goalOptions.map((option) => {
              ...
              return (
                <label key={option.key} className="flex-1 cursor-pointer">
                  ...
                  <div className={`flex items-center justify-center py-2.5 rounded-lg text-sm font-medium transition-all ${isSelected ? 'bg-Malama-petrol dark:bg-primary text-white shadow-md' : 'text-Malama-muted dark:text-gray-400'}`}>
                    {option.label}
                  </div>
                </label>
              );
        ...
        <div className="px-5 mt-8 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="bg-white dark:bg-surface-dark p-5 rounded-2xl shadow-sm border border-Malama-border dark:border-white/10">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-Malama-petrol dark:text-primary font-bold text-lg">{frequency}x</span>
            </div>
          ...
          <div className="mt-3 flex items-center justify-between bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-Malama-border dark:border-white/10 cursor-pointer hover:bg-gray-50 dark:hover:bg-Malama-dark transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-Malama-muted dark:text-gray-500">{rp.preferredTimeDesc}</span>
              </div>
            </div>
            <div className="flex items-center gap-2 font-semibold">
              07:00 AM
              <span className="material-symbols-outlined text-Malama-muted text-sm">edit</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-Malama-border dark:border-white/10 flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="flex items-baseline gap-1 relative z-10">
                ...
                <span className="text-sm text-Malama-muted">%</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 mt-8 mb-6 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
          <div className="bg-white dark:bg-surface-dark rounded-[2rem] p-6 shadow-lg border border-Malama-border dark:border-white/10 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-Malama-petrol/10 dark:bg-primary/10 px-2 py-1 text-xs font-bold text-Malama-petrol dark:text-primary tracking-wide">
            ...
            <div className="text-center mb-8 relative z-10">
              <span className="block text-[10px] text-Malama-muted uppercase font-bold tracking-widest mb-1">{rp.dailyGoal}</span>
              <div className="flex items-center justify-center gap-1">
                ...
                <div className="flex flex-col items-start justify-center h-full pt-3">
                  <span className="text-[10px] font-bold text-Malama-muted">KCAL</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 relative z-10">
              {[
                { label: rp.protein, value: `${targetProt}g`, pct: Math.round((targetProt * 4 / targetCal) * 100), color: 'text-Malama-petrol dark:text-primary', icon: 'egg_alt' },
                ...
                <div key={macro.label} className="flex flex-col items-center">
                  <div className="relative size-14 mb-2">
                    <svg className="size-full -rotate-90" viewBox="0 0 36 36">
                      <path className="text-Malama-border dark:text-gray-700" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="2.5"></path>
                  ...
                  <span className="text-xs font-medium text-Malama-muted">{macro.label}</span>
                </div>
              ))}
            </div>
          </div>
