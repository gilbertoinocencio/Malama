...
export const PlanRenewal: React.FC<PlanRenewalProps> = ({ onBack, onNavigate }) => {
  ...
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col max-w-md mx-auto overflow-x-hidden shadow-2xl bg-Malama-bg dark:bg-background-dark font-display antialiased transition-colors duration-300 text-Malama-main dark:text-white animate-fade-in">

      {/* Header */}
      <header className="flex items-center justify-between p-6 pt-8 pb-2 sticky top-0 z-30 bg-Malama-bg/95 dark:bg-background-dark/95 backdrop-blur-sm">
        ...
        <div className="flex flex-col items-center">
          <span className="text-xs font-bold uppercase tracking-[0.15em] text-Malama-muted dark:text-gray-400">{pr.header}</span>
        </div>
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar pb-24">
        ...
        <section className="px-6 pt-6 pb-4 text-center animate-fade-in-up">
          <h1 className="text-[34px] font-light leading-[1.15] tracking-tight mb-3">
            {pr.heroTitle} <br /> <span className="font-bold text-Malama-petrol dark:text-primary">{pr.heroAccent}</span>
          </h1>
          <p className="text-Malama-muted dark:text-gray-400 text-base font-normal leading-relaxed max-w-[280px] mx-auto">{pr.heroSubtitle}</p>
        </section>
          ...
          <div className="bg-white dark:bg-surface-dark rounded-2xl p-6 shadow-sm border border-Malama-border dark:border-white/10 flex flex-col gap-4 relative overflow-hidden">
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="flex flex-col gap-1">
                <span className="text-Malama-muted dark:text-gray-400 text-sm font-medium tracking-wide uppercase">{pr.consistency}</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-Malama-petrol dark:text-primary text-4xl font-light tracking-tighter">{consistency}%</span>
                  {gameStats && gameStats.currentStreak > 0 && (
                    <span className="inline-flex items-center text-Malama-main dark:text-gray-300 text-sm font-bold bg-Malama-petrol/10 dark:bg-primary/10 px-2 py-0.5 rounded-full">
              ...
              <div className="w-12 h-12 relative flex items-center justify-center">
                <svg className="transform -rotate-90 w-full h-full" viewBox="0 0 36 36">
                  <path className="text-Malama-border dark:text-gray-700" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="4"></path>
                  <path className="text-Malama-petrol dark:text-primary" strokeDasharray={`${consistency}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round"></path>
                </svg>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center text-sm relative z-10">
              ...
              <div className="flex -space-x-2">
                <div className="w-6 h-6 rounded-full bg-Malama-petrol dark:bg-primary flex items-center justify-center border-2 border-white dark:border-[#1a2630] text-white shadow-sm z-20">
                  <span className="material-symbols-outlined text-[12px]">star</span>
                </div>
                <div className="w-6 h-6 rounded-full bg-Malama-border dark:bg-gray-600 border-2 border-white dark:border-[#1a2630] z-10"></div>
                <div className="w-6 h-6 rounded-full bg-Malama-border dark:bg-gray-600 border-2 border-white dark:border-[#1a2630] z-0"></div>
              </div>
            </div>
          </div>
        </section>
      ...
      {/* Bottom Action */}
      <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-Malama-bg via-Malama-bg/95 to-transparent dark:from-background-dark dark:via-background-dark/95 pt-12">
        <button
