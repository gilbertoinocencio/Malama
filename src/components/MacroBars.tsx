...
export const MacroBars: React.FC<MacroBarsProps> = ({ current, target }) => {
  ...
  const macros = [
    { label: 'Protein', key: 'protein' as keyof MacroData, color: 'bg-Malama-petroleum dark:bg-Malama-cyan' },
    { label: 'Carbs', key: 'carbs' as keyof MacroData, color: 'bg-Malama-brown dark:bg-Malama-neon' },
    ...
    <div className="flex gap-4 w-full justify-between mt-6">
      {macros.map((m) => (
        <div key={m.label} className="flex flex-col flex-1 gap-1 group">
          ...
          <div className="h-2 w-full bg-Malama-stone/30 dark:bg-white/10 rounded-full overflow-hidden">
            <div 
