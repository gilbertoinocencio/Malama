...
const TimeControl: React.FC<{
  ...
  <div className={`p-8 rounded-lg border-2 transition-all duration-300 ${isActive ? 'bg-surface-container-lowest border-tertiary-fixed-dim shadow-md' : 'bg-surface-container-low border-transparent'}`}>
    <div className="flex justify-between items-start mb-6">
      <p className="font-headline text-tertiary-container font-semibold uppercase text-xs tracking-widest">{label}</p>
      {isActive && (
    ...
    <div className="flex items-end gap-2">
      ...
      <div className="flex flex-col items-center gap-2">
        <button
          ...
          className="w-8 h-8 rounded-full flex items-center justify-center text-tertiary hover:bg-surface-container-high transition-all active:scale-90"
        >
          <span className="material-symbols-outlined text-lg">expand_less</span>
        </button>
        <span className="font-headline text-6xl text-tertiary tracking-tighter w-[2.5ch] text-center">{fmt(hour)}</span>
        <button
          onClick={() => onHourChange(-1)}
          className="w-8 h-8 rounded-full flex items-center justify-center text-tertiary hover:bg-surface-container-high transition-all active:scale-90"
        >
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <span className="font-headline text-6xl text-tertiary tracking-tighter w-[2.5ch] text-center">{fmt(minute)}</span>
        <button
          onClick={() => onMinuteChange(-15)}
          className="w-8 h-8 rounded-full flex items-center justify-center text-tertiary hover:bg-surface-container-high transition-all active:scale-90"
        >
    ...
    <div className={`mt-6 h-0.5 w-full transition-all duration-500 ${isActive ? 'bg-tertiary' : 'bg-surface-container-highest'}`}></div>
    ...
    >
      ...
      <div className="w-full max-w-xl space-y-10 relative z-10">
        <div className="space-y-4">
          ...
          <h1 className="font-headline text-4xl md:text-5xl text-tertiary font-bold tracking-tight leading-tight">
        ...
        <div className="flex items-center justify-center gap-3 py-2">
          ...
          <span className="font-headline font-semibold text-tertiary">
