  ...
  return (
    ...
    >
      <div className="mb-12 space-y-4">
        <h2 className="font-headline text-4xl md:text-5xl text-tertiary font-bold leading-tight tracking-tight">Onde você costuma comer?</h2>
          ...
          return (
            <button
              ...
              className={`w-full group transition-all duration-300 ease-in-out p-8 rounded-lg text-left flex items-center gap-8 border-2 ${
                isSelected 
                  ? 'bg-tertiary-fixed-dim border-tertiary-container/20 ring-2 ring-tertiary-container/10 shadow-lg' 
            ...
            >
              <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ${isSelected ? 'bg-surface-container-lowest' : 'bg-surface-container-highest'}`}>
                <span className={`material-symbols-outlined text-3xl ${isSelected ? 'text-tertiary' : 'text-tertiary/70'}`}>
                  {local.icon}
                </span>
              </div>
              <div className="flex-grow">
                <p className={`font-headline text-xl font-medium ${isSelected ? 'text-tertiary' : 'text-tertiary/80'}`}>
                  {local.label}
                </p>
              </div>
