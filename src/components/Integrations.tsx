...
export const Integrations: React.FC<IntegrationsProps> = ({ onBack }) => {
  ...
  const [items, setItems] = useState<IntegrationItem[]>([
    ...
    { id: 'samsung', name: 'Samsung Health', icon: 'vital_signs', status: 'disconnected', color: 'text-[#1428a0] dark:text-indigo-400', iconBg: 'bg-indigo-50', darkIconBg: 'dark:bg-indigo-900/20', opacity: 'opacity-80' },
  ]);
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'connected': return ig.connected;
      case 'pending': return ig.pending;
      case 'disconnected': return ig.disconnected;
      default: return '';
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'connected': return 'text-Malama-petrol dark:text-primary';
      case 'pending': return 'text-Malama-muted dark:text-slate-400';
      case 'disconnected': return 'text-Malama-muted/70 dark:text-slate-600';
      default: return '';
    }
  };
          ...
          <div key={item.id} className={`flex items-center gap-4 bg-white dark:bg-surface-dark px-4 py-4 rounded-xl shadow-sm dark:shadow-none border border-Malama-border dark:border-white/5 transition-all hover:shadow-md ${item.opacity || ''}`}>
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="flex flex-col">
                ...
                <p className={`text-xs font-medium ${getStatusColor(item.status)}`}>{getStatusText(item.status)}</p>
              </div>
            </div>
            <div className="shrink-0">
              <label className={`relative flex h-[31px] w-[51px] cursor-pointer items-center rounded-full border-none p-0.5 transition-colors duration-200 ease-in-out ${item.status === 'connected' ? 'justify-end bg-Malama-petrol dark:bg-primary' : 'justify-start bg-Malama-petrol/20 dark:bg-primary/20'}`}>
                <input type="checkbox" className="invisible absolute" checked={item.status === 'connected'} onChange={() => toggleStatus(item.id)} />
