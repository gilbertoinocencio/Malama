...
export const VisualShare: React.FC<VisualShareProps> = ({ onBack }) => {
  ...
  const handleDownload = async () => {
    if (cardRef.current) {
      ...
      link.download = `Malama-evolution-${new Date().toISOString().split('T')[0]}.png`;
  ...
  const renderConsistencyGrid = (opacity: string) => (
    <div className={`grid grid-cols-12 gap-[3px] ${opacity}`}>
      {Array.from({ length: 12 }).map((_, i) => (
        <div key={i} className={`aspect-square rounded-[1px] ${i === 3 && opacity.includes('60') ? 'bg-Malama-petrol/40' : 'bg-Malama-petrol'}`}></div>
      ))}
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#e0e0e0] dark:bg-[#0a0a0a] flex flex-col justify-center items-center overflow-hidden font-sans animate-fade-in">
      ...
      >
        {/* Border Frame */}
        <div className="absolute inset-4 border border-Malama-border/30 z-20 pointer-events-none rounded-[2px]"></div>
        ...
        <div className="flex-1 flex flex-col px-8 py-10 z-10 h-full justify-between">
          ...
          <div className="text-center mt-10 relative">
            <div className="w-px h-8 bg-Malama-border/30 mx-auto mb-4"></div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-Malama-border mb-3 font-medium">Jornada Feed the Flow</p>
            <h1 className="font-serif text-5xl text-Malama-petrol leading-[1.05]">
              90 Dias<br/>
              <span className="italic font-light text-Malama-dark text-4xl">de Flow</span>
            </h1>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center my-6">
            <div className="grid grid-cols-2 gap-4 w-full">
              ...
              <div className="flex flex-col gap-3 group">
                <div className="relative aspect-[9/16] bg-white border border-Malama-border/20 shadow-frame p-1.5">
                ...
                <div className="flex items-center justify-center gap-2 opacity-80">
                  <span className="font-serif italic text-Malama-border text-sm">Início</span>
                </div>
              </div>
              
              {/* After */}
              <div className="flex flex-col gap-3 group">
                <div className="relative aspect-[9/16] bg-white border border-Malama-petrol/20 shadow-frame p-1.5">
                  ...
                  <div className="absolute -top-1 -right-1 size-2.5 bg-Malama-petrol rounded-full border-2 border-Malama-bg shadow-sm"></div>
                </div>
                <div className="flex items-center justify-center gap-2">
                  <span className="font-serif italic text-Malama-petrol text-sm font-medium">Atual</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8 mb-6">
            <div className="space-y-3">
              <div className="flex justify-between items-end text-Malama-petrol px-1 border-b border-Malama-border/10 pb-2">
            ...
            <div className="text-center pt-2">
              <div className="inline-block border-y border-Malama-border/20 py-2 px-6">
                <h3 className="font-serif font-bold text-xl text-Malama-dark tracking-tight leading-none">Malama</h3>
                <p className="text-[8px] tracking-[0.45em] uppercase text-Malama-border mt-1">Feed the Flow</p>
              </div>
            </div>
          </div>
        </div>
