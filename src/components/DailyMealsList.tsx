  ...
  return (
    <div className="px-6 mb-4 mt-2">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-nura-main dark:text-white text-lg font-bold">Refeições de Hoje</h2>
      </div>
      
      <div className="bg-white dark:bg-surface-dark rounded-xl p-5 shadow-sm border border-nura-border dark:border-transparent transition-colors duration-300 flex flex-col gap-6">
        {todaysMeals.map((meal, index) => (
          <div key={meal.id} className="flex flex-col gap-3 relative">
            {index !== todaysMeals.length - 1 && (
              <div className="absolute left-[19px] top-10 bottom-[-24px] w-[2px] bg-nura-border dark:bg-white/5 z-0" />
            )}

            {/* Cabeçalho da refeição */}
            <div className="flex items-start gap-4 relative z-10 w-full group">
              <div 
                className="size-10 rounded-full bg-nura-bg dark:bg-white/5 border border-nura-border dark:border-white/10 flex items-center justify-center shrink-0 cursor-pointer"
              ...
              <div className="flex-1 min-w-0 pt-0.5" onClick={() => toggleExpand(meal.id)}>
                <div className="flex justify-between items-center mb-1">
                  <h3 className="text-nura-main dark:text-white font-bold text-base truncate pr-2 capitalize cursor-pointer">
                  ...
                  <div className="flex gap-2 items-center opacity-0 group-hover:opacity-100 transition-opacity absolute right-6 top-0 bg-white dark:bg-surface-dark px-2 rounded-lg shadow-sm">
                    <button onClick={(e) => { e.stopPropagation(); setEditingMeal({...meal}); }} className="text-nura-petrol dark:text-primary p-1 hover:bg-nura-petrol/10 rounded">
                  ...
                  <span className="material-symbols-outlined text-nura-muted text-sm cursor-pointer ml-auto">
                    {expandedMeals[meal.id] ? 'expand_less' : 'expand_more'}
                  </span>
                </div>
                
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 cursor-pointer">
                  <span className="text-sm font-bold text-nura-petrol dark:text-primary shrink-0">
                    {meal.calories} <span className="text-[10px] font-normal uppercase tracking-wider">kcal</span>
                  </span>
                  <div className="flex gap-2">
                    <span className="text-[10px] font-medium text-nura-muted dark:text-slate-500">P <strong className="text-nura-main dark:text-slate-300">{meal.macros.protein}g</strong></span>
                    <span className="text-[10px] font-medium text-nura-muted dark:text-slate-500">C <strong className="text-nura-main dark:text-slate-300">{meal.macros.carbs}g</strong></span>
                    <span className="text-[10px] font-medium text-nura-muted dark:text-slate-500">G <strong className="text-nura-main dark:text-slate-300">{meal.macros.fats}g</strong></span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {expandedMeals[meal.id] && meal.items && meal.items.length > 0 && (
              <div className="ml-[56px] flex flex-col gap-1.5 relative z-10 animate-fade-in-up">
                {meal.items.map((item, idx) => (
                  <div key={idx} className="bg-nura-bg dark:bg-[#152226] rounded-md p-2 border border-transparent hover:border-nura-petrol/10 dark:hover:border-primary/10 transition-colors text-xs text-nura-muted dark:text-slate-400">
      ...
      {editingMeal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-surface-dark w-full max-w-sm rounded-[24px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-nura-border dark:border-white/5 flex items-center justify-between bg-nura-bg dark:bg-white/5">
              <h3 className="font-bold text-nura-main dark:text-white">Editar Refeição</h3>
              <button onClick={() => setEditingMeal(null)} className="text-nura-muted hover:text-red-500 transition-colors">
                <span className="material-symbols-outlined text-xl">close</span>
              </button>
            </div>
            
            <div className="p-5 flex-1 overflow-y-auto no-scrollbar flex flex-col gap-4">
              <label className="flex flex-col gap-1 text-sm font-semibold text-nura-main dark:text-white">
                Nome da Refeição
                <input 
                  type="text" 
                  value={editingMeal.name} 
                  onChange={e => setEditingMeal({...editingMeal, name: e.target.value})}
                  className="bg-nura-bg dark:bg-[#152226] border border-nura-border dark:border-white/10 rounded-xl px-4 py-2 text-nura-main dark:text-white focus:border-nura-petrol dark:focus:border-primary outline-none transition-colors"
                />
              </label>

              <div className="flex flex-col gap-3 mt-2">
                <h4 className="text-xs font-bold text-nura-muted dark:text-slate-400 uppercase tracking-wider">Itens</h4>
                {editingMeal.items?.map((item, idx) => (
                  <div key={idx} className="p-3 bg-nura-bg dark:bg-[#152226] rounded-xl border border-nura-border dark:border-white/5 flex flex-col gap-2 relative">
                    <input 
                      type="text" 
                      placeholder="Nome"
                      value={item.name} 
                      onChange={e => handleItemChange(idx, 'name', e.target.value)}
                      className="bg-white dark:bg-surface-dark w-full border border-nura-border dark:border-white/10 rounded-lg px-3 py-1.5 text-xs text-nura-main dark:text-white outline-none"
                    />
                    <div className="flex gap-2">
                      <div className="flex-1 flex flex-col gap-1">
                        <span className="text-[9px] text-nura-muted uppercase font-bold">Peso/Qtd (g)</span>
                        <input 
                          type="number" 
                          value={item.weightGrams || ''} 
                          onChange={e => handleItemChange(idx, 'weightGrams', e.target.value)}
                          className="bg-white dark:bg-surface-dark w-full border border-nura-border dark:border-white/10 rounded-lg px-2 py-1 text-xs text-nura-main dark:text-white outline-none"
                        />
                      </div>
                ...
                <button 
                  ...
                  className="w-full py-2 border-2 border-dashed border-nura-petrol/30 dark:border-primary/30 text-nura-petrol dark:text-primary font-semibold text-xs rounded-xl flex items-center justify-center gap-1 active:scale-95 transition-all"
                >
            ...
            <div className="p-4 bg-nura-bg dark:bg-white/5 border-t border-nura-border dark:border-white/5">
              <button 
                onClick={handleSaveEdit}
                className="w-full bg-nura-petrol dark:bg-primary text-white font-bold h-12 rounded-xl active:scale-[0.98] transition-all"
              >
                Salvar Alterações
              </button>
            </div>
