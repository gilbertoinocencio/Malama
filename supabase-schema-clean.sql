...
BEGIN
  ...
  FOR r IN (
    ...
    WHERE schemaname = 'public'
      AND tablename IN (
        'profiles', 'meals', 'daily_logs', 'flow_stats', 'posts',
        'achievements', 'nutritionist_onboarding', 'quarterly_plans'
      )
  ) LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON %I.%I', r.policyname, r.schemaname, r.tablename);
  END LOOP;

  -- Drop all policies on storage.objects for meal-photos
  FOR r IN (
    SELECT policyname
    FROM pg_policies
    WHERE schemaname = 'storage'
      AND tablename = 'objects'
      AND policyname LIKE '%meal photo%'
  ) LOOP
...
CREATE INDEX idx_flow_stats_user_date ON flow_stats(user_id, date DESC);
...
CREATE INDEX idx_onboarding_user ON nutritionist_onboarding(user_id);
CREATE INDEX idx_plans_user_status ON quarterly_plans(user_id, status);
