-- =====================================================
-- Malama — Publica os Termos de Uso do Painel do RH
-- Migration: 20260828_publica_termos_painel_rh.sql
--
-- Aplicar via SQL Editor. Substitui o texto de 20260826 e publica.
--
-- Três decisões do dono do produto entraram aqui:
--   · foro: comarca de Taubaté/SP;
--   · prazo de comunicação de incidente: 72 horas;
--   · razão social: "Malama Saúde Digital Ltda." NÃO EXISTE. A entidade é
--     "Malama Healthtech". O nome errado também estava nas páginas públicas
--     de termos, privacidade e exclusão de conta, e foi corrigido lá.
--
-- Esta migration é autocontida: se a linha de 'termos_b2b' não existir (por
-- 20260825/20260826 não terem rodado), ela é criada antes do UPDATE. Rodar
-- duas vezes é inofensivo.
--
-- A PARTIR DAQUI O DOCUMENTO ESTÁ VIGENTE e aparece no Portal do RH para
-- aceite. Publicar uma versão nova depois disto = INSERIR outra linha com
-- versão nova e baixar esta (vigente = FALSE); o índice parcial só admite
-- uma vigente por tipo, e os aceites da anterior ficam preservados.
-- =====================================================

-- Garante a linha antes de escrever o texto final.
INSERT INTO documentos_legais (tipo, versao, titulo, conteudo, exige_aceite, vigente)
SELECT 'termos_b2b', '1.0', 'Termos de Uso do Painel do RH — Empresa Contratante',
       'Aguardando publicação.', TRUE, FALSE
WHERE NOT EXISTS (
